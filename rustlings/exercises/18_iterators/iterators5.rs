...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize{
    ...
    // let values: Vec<&Progress> = map.values().collect();
    // let values_iter = values.into_iter();
    let p = map.values().fold(0, |acc,elem| {
        if *elem == value {
            acc+1
        } else {
            acc
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize{
    ...
    //               { "variables2": Complete, … }, … ]
    let collection_vec = collection.into_iter().map(|x| {count_iterator(x,value)}).collect::<Vec<usize>>();
    let sum: usize = collection_vec.iter().sum();
    //println!("{}", sum);
    sum
